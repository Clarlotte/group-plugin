...
export class update extends plugin {
    constructor() {
        super({
            name: '[group-plugin] 插件更新',
            dsc: '[group-plugin] 插件更新',
            event: 'message',
            priority: 1,
            rule: [
                {
    ...
    async getLog(plugin = '') {
        ...
        return common.makeForwardMsg(this.e, [log, end], `${plugin || 'group-plugin'} 更新日志，共${line}条`)
    }

    async updateLog() {
        let log = await this.getLog('group-plugin')
        await this.reply(log)
    }
